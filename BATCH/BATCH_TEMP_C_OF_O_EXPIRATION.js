/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_TEMP_C_OF_O_EXPIRATION.js  Trigger: Batch
| Client:
|
...
var conditionType = getParam("conditionType"); // standard condition type to apply to cap
var conditionName = getParam("conditionName"); // standard condition name to apply to cap
/*----------------------------------------------------------------------------------------------------/
|
...
//			logDebug(altId + ": skipping due to application status of " + capStatus)
		...
		var tempCOExpiration = getAppSpecific("Temp C of O Expiration Date", capId);
		if (tempCOExpiration == null) {
			capFilterExpirationDate++;
//			logDebug(altId + ": unable to get ASI 'Application Expiration'");
			continue;
		}
		var dtempCOExpiration = new Date(tempCOExpiration);
		if (!dtempCOExpiration){
			capFilterExpirationDate++;
			logDebug(altId + ": unable to create Date object from ASI 'Temp C of O Expiration Date'");
			continue;			
		}
		if (isNaN(dtempCOExpiration.getTime())){
			capFilterExpirationDate++;
			logDebug(altId + ": unable to create valid Date object from ASI 'Temp C of O Expiration Date'");
			continue;			
		}
		var coIssued = getAppSpecific("C of O Issued", capId);
		if (!!coIssued){
			capFilterExpirationDate++;
			continue;
		}
		...
		if (isNaN(dActualTempCOExpirationDate.getTime())){
			capFilterExpirationDate++;
			logDebug(altId + ": unable to create valid Date object for actual Temp C of O Expiration including grace period");
			continue;			
		}
		if (dActualTempCOExpirationDate.getTime() < dtempCOExpiration.getTime()) {
			capFilterExpirationDate++;
//			logDebug(altId + ": skipping, temp C of O has not expired beyond grace period");
			continue;
		}
		...
		// add condition
		addStdCondition(conditionType, conditionName, capId);
